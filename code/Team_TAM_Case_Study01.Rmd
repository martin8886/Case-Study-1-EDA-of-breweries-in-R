...
MedianResults %>% 
  group_by(MedianResults$State) %>% 
  summarize(Max_ABV = max(Median_ABV, na.rm = TRUE), Max_IBU = max(Median_IBU, na.rm = TRUE)) -> MaxResults
...
#The state with the most alcoholic content is DC.
#The state with the highest IBU is Maine
